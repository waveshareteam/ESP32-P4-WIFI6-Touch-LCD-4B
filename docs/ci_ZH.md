...
以下稳定版本已于 2026-08-08 重新核验，并使用固定版本而非可移动别名：
...
| 选择器或跨框架策略输入 | 全部 26 个构建 | 全部 5 个编译 |
...
`Repository Policy` 使用 Python 标准库单元测试验证路由契约。测试覆盖预期的
13/5 清单、纯文档变更、直属与共享输入、固件边界、未知输入回退、重命名、复制、
删除、空范围以及精确的手动选择器调用。合成打包测试还会验证完整烧录文件收集、
清单哈希与顺序、不安全路径拒绝和覆盖保护；这些测试不会编译固件。
...
默认工作流验证源码编译，不发布固件制品。打包属于独立的发布边界。需要发布包时，
`scripts/package_esp_idf_firmware.py` 会从 `flasher_args.json` 获取文件名和偏移；
Brookesia 软件包还必须包含所有被引用的模型或文件系统镜像。

CI 构建包不等同于工厂或恢复镜像。硬件 PDF、结构图和导入的资源归档绝不能混入
固件制品。生成的软件包继续保存在 `release-artifacts/` 或 `releases/dist/` 等
已忽略路径下。
