...
#define DSI_STEP_CHECK(expr, step) do {                \
    esp_err_t _err = (expr);                           \
    if (_err != ESP_OK) {                              \
      Serial.printf("DSI init failed at %s: 0x%x\n", step, (unsigned)_err); \
      return false;                                    \
    }                                                  \
  } while (0)
...
bool Arduino_ESP32DSIPanel::begin(int16_t w, int16_t h, int32_t speed, const lcd_init_cmd_t *init_operations, size_t init_operations_len)
{
  ...
  DSI_STEP_CHECK(esp_ldo_acquire_channel(&ldo_mipi_phy_config, &ldo_mipi_phy), "ldo");
  ...
  DSI_STEP_CHECK(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus), "dsi-bus");
  ...
  DSI_STEP_CHECK(esp_lcd_new_panel_io_dbi(mipi_dsi_bus, &dbi_config, &io_handle), "dbi-io");
  ...
  DSI_STEP_CHECK(esp_lcd_new_panel_dpi(mipi_dsi_bus, &dpi_config, &_panel_handle), "dpi-panel");

  for (int i = 0; i < init_operations_len; i++)
  {
    // Send command
    DSI_STEP_CHECK(esp_lcd_panel_io_tx_param(io_handle, init_operations[i].cmd, init_operations[i].data, init_operations[i].data_bytes), "init-cmd");
    vTaskDelay(pdMS_TO_TICKS(init_operations[i].delay_ms));
  }
  ESP_LOGD(TAG, "send init commands success");

  DSI_STEP_CHECK(esp_lcd_panel_init(_panel_handle), "panel-init");
...
uint16_t *Arduino_ESP32DSIPanel::getFrameBuffer()
{
  ...
  if (esp_lcd_dpi_panel_get_frame_buffer(_panel_handle, 1, &frame_buffer) != ESP_OK) {
    Serial.println("DSI framebuffer request failed");
    return nullptr;
  }
