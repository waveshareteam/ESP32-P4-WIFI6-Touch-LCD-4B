...
static void transfer_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        if (!usr_frame) {
            ESP_LOGE(TAG, "filled frame queue receive failed");
            continue;
        }
        app_lcd_draw(usr_frame->data,  usr_frame->info.total, usr_frame->info.width, usr_frame->info.height);
        if (frame_return_empty(usr_frame) != ESP_OK) {
            ESP_LOGE(TAG, "return empty frame failed");
        }
    }
}
...
{
    if (frame_info->received >= frame_info->total || len >= frame_info->total - frame_info->received) {
        frame_info->received = frame_info->total;
        return true;
    }
...
static bool buffer_fill(frame_t *frame, uint8_t *buf, uint32_t len)
{
    ...
    if (frame->info.received > frame->info.total || len > frame->info.total - frame->info.received) {
        ESP_LOGW(TAG, "Drop frame: data exceeds declared payload, total=%"PRIu32", received=%"PRIu32", len=%"PRIu32,
                 frame->info.total, frame->info.received, len);
        (void)frame_return_empty(frame);
        return true;
    }

    esp_err_t add_result = frame_add_data(frame, buf, len);
    ...
    if (frame->info.received >= frame->info.total) {
        esp_err_t send_result = frame_send_filled(frame);
        if (send_result != ESP_OK) {
            ESP_LOGE(TAG, "queue filled frame failed");
            (void)frame_return_empty(frame);
        }
        return true;
    }
    ...
    while (tud_vendor_n_available(itf)) {
        ...
        if (read_res > 0) {
            if (!current_frame && !skip_frame) {
                ...
                const udisp_frame_header_t *pblt = &header;
                uint32_t first_payload_len = (uint32_t)read_res - sizeof(udisp_frame_header_t);
                switch (pblt->type) {
                ...
                case UDISP_TYPE_END:
                    break;
                default:
                    ESP_LOGW(TAG, "Drop unknown frame type: %u", pblt->type);
                    skip_frame = start_skip_frame(&skip_frame_info, pblt->payload_total, first_payload_len);
                    break;
                }
            } else if (skip_frame) {
...
{
    ESP_RETURN_ON_ERROR(frame_allocate(6, CONFIG_USB_EXTEND_SCREEN_FRAME_LIMIT_B), TAG,
                        "allocate frame buffers failed");
    BaseType_t created = xTaskCreatePinnedToCore(transfer_task, "transfer_task", 4096, NULL,
                                                 CONFIG_VENDOR_TASK_PRIORITY, NULL, 1);
    ESP_RETURN_ON_FALSE(created == pdPASS, ESP_ERR_NO_MEM, TAG, "create transfer task failed");
    return ESP_OK;
}
