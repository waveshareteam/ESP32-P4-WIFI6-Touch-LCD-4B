...
esp_err_t app_usb_init(void)
{
    ...
    BaseType_t created = xTaskCreate(tusb_device_task, "tusb_device_task", 4096, NULL,
                                     CONFIG_USB_TASK_PRIORITY, NULL);
    ESP_RETURN_ON_FALSE(created == pdPASS, ESP_ERR_NO_MEM, TAG, "create TinyUSB task failed");
    return ret;
}
