...
void tinyusb_hid_keyboard_report(hid_report_t report)
{
    if (!s_tinyusb_hid || !s_tinyusb_hid->hid_queue) {
        ESP_LOGW(TAG, "HID is not initialized");
        return;
    }
    // Remote wakeup
    if (tud_suspended()) {
        ...
        tud_remote_wakeup();
    } else if (xQueueSend(s_tinyusb_hid->hid_queue, &report, 0) != pdPASS) {
        ESP_LOGW(TAG, "HID report queue is full");
    }
}
        ...
        if (xQueueReceive(s_tinyusb_hid->hid_queue, &report, portMAX_DELAY)) {
            ...
            if (tud_suspended()) {
                ...
                if (!tud_hid_n_report(0, REPORT_ID_TOUCH, &report.touch_report, sizeof(report.touch_report))) {
                    ESP_LOGW(TAG, "HID report submission failed");
                    continue;
                }
...
esp_err_t app_hid_init(void)
{
    ...
    BaseType_t created = xTaskCreate(tinyusb_hid_task, "tinyusb_hid_task", 4096, NULL,
                                     CONFIG_HID_TASK_PRIORITY, &s_tinyusb_hid->task_handle);
    ESP_GOTO_ON_FALSE(created == pdPASS, ESP_ERR_NO_MEM, fail, TAG, "create HID task failed");
    return ESP_OK;
fail:
