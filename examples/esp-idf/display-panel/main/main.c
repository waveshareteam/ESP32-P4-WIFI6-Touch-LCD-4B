...
void touch_event_handler(lv_event_t *e)
{
    ...
    if (touch_point.x >= 0 && touch_point.x < BSP_LCD_H_RES && touch_point.y >= 0 && touch_point.y < BSP_LCD_V_RES)
    {
...
static void show_color(lv_color_t color)
{
    ESP_ERROR_CHECK(bsp_display_lock(-1) ? ESP_OK : ESP_ERR_TIMEOUT);
    fill_screen_with_color(color);
    bsp_display_unlock();
...
void app_main(void)
{
    ...
    show_color(LV_COLOR_RED);
    show_color(LV_COLOR_GREEN);
    show_color(LV_COLOR_BLUE);
    ...
    lv_obj_set_size(touch_area, BSP_LCD_H_RES, BSP_LCD_V_RES);
