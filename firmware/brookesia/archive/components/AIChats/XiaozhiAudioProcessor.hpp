...
/**
 * Single-microphone WakeNet front end for the Xiaozhi app.
 *
 * Input is 16 kHz signed 16-bit mono PCM (AFE input format "M"). AEC is not
 * required for wake-word detection or basic server conversations. The PCM
 * callback receives one 60 ms mono frame at a time. Its data pointer is valid
 * only for the duration of the callback.
 */
class XiaozhiAudioProcessor {
public:
    static constexpr uint32_t SAMPLE_RATE = 16000;
    static constexpr size_t INPUT_CHANNELS = 1;
    ...
    /** Feed frame_count mono samples into the AFE. */
    bool feed(const int16_t *mono_pcm, size_t frame_count);
