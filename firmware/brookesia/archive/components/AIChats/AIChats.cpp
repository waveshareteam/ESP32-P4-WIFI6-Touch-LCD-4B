...
#endif
#define ESP_UTILS_LOG_TAG "BS:AIChats"
#include "esp_lib_utils.h"

LV_IMG_DECLARE(img_app_aichats);
...
AIChats *AIChats::_instance = nullptr;

AIChats *AIChats::requestInstance(bool use_status_bar, bool use_navigation_bar)
{
    if (!_instance) {
        _instance = new AIChats(use_status_bar, use_navigation_bar);
    }
    return _instance;
}

AIChats::AIChats(bool use_status_bar, bool use_navigation_bar):
    App("Xiaozhi", &img_app_aichats, true, use_status_bar, use_navigation_bar)
{
...
void AIChats::createUi()
{
    ...
    if (!_ui->create(_page_root, uiActionCallback, this, false)) {
        destroyUi();
        return;
    }
...
void AIChats::refreshUi()
{
    ...
    memcpy(error, _last_error, sizeof(error));
    xSemaphoreGive(_state_mutex);

    State state = _state.load();
    _ui->setNetworkReady(_network_ready.load());
    _ui->setStatus(
        state == State::Error && error[0] ? error : stateText(state)
    );
    _ui->setChatMessage(role, text);
    _ui->setEmotion(emotion[0] ? emotion : "neutral");
    _ui->setActivation(
        code,
        message[0] ? message : "请在小智控制面板添加设备并输入激活码。",
        state == State::ActivationRequired
    );
}

void AIChats::setState(State state)
{
    _state.store(state);
}

void AIChats::setError(esp_err_t error, const char *source)
{
...
void AIChats::setChatMessage(XiaozhiProtocol::TextRole role, const char *text)
{
    if (!_state_mutex || xSemaphoreTake(_state_mutex, portMAX_DELAY) != pdTRUE) {
        return;
    }
    copyString(
        _chat_role,
        sizeof(_chat_role),
        role == XiaozhiProtocol::TextRole::Assistant ? "assistant" : "user"
    );
    copyString(_chat_text, sizeof(_chat_text), text);
    xSemaphoreGive(_state_mutex);
}

void AIChats::setEmotion(const char *emotion)
{
    if (_state_mutex && xSemaphoreTake(_state_mutex, portMAX_DELAY) == pdTRUE) {
...
void AIChats::clearConversationUi()
{
    if (_state_mutex && xSemaphoreTake(_state_mutex, portMAX_DELAY) == pdTRUE) {
        _chat_role[0] = '\0';
        _chat_text[0] = '\0';
...
void AIChats::copyActivation(const char *code, const char *message)
{
    if (_state_mutex && xSemaphoreTake(_state_mutex, portMAX_DELAY) == pdTRUE) {
...
bool AIChats::announceActivation(bool force)
{
...
void AIChats::stopActivationPrompt()
{
    if (!_activation_prompt) {
        return;
    }
    _activation_prompt->cancel();
    while (!_activation_prompt->stopAndWait(1000)) {
        ESP_UTILS_LOGW("Waiting for Xiaozhi prompt audio to stop");
    }
}

bool AIChats::playReadyPromptOnce()
{
...
bool AIChats::postControlEvent(const ControlEvent &event, bool wait_for_ack)
{
    ScopedLifecycleAdmission lifecycle_user(_lifecycle_refs);
    if (!lifecycle_user) {
        return false;
...
void AIChats::handleControlEvent(const ControlEvent &event)
{
    ...
    case ControlEventType::WakeWord:
    {
        ...
        XiaozhiProtocol *protocol = _protocol.load();
        if (_visible.load() && state == State::Ready) {
            ...
            if (_audio_tx_mutex &&
                ...
                abort_ret = protocol->sendAbortSpeaking(
                    XiaozhiProtocol::AbortReason::WakeWordDetected
                );
    ...
    case ControlEventType::TransportConnected:
        ...
        if (_chat_started.load() &&
            ...
            setState(State::Ready);
            if (_visible.load() && ensureAudioReady()) {
                enterReadyAudioMode();
            } else if (_visible.load()) {
                schedulePrepare(RETRY_INTERVAL_MS);
            }
    ...
    case ControlEventType::ChatText:
        if (_channel_state.load() == ChannelState::Open &&
            ...
            setChatMessage(
                static_cast<XiaozhiProtocol::TextRole>(event.role), event.text
            );
        }
        break;
...
void AIChats::handleAppHidden(bool release_audio)
{
    _visible.store(false);
    stopActivationPrompt();
    setAudioModes(false, false);
...
void AIChats::schedulePrepare(uint32_t delay_ms)
{
    _next_prepare_tick = xTaskGetTickCount() + pdMS_TO_TICKS(delay_ms);
}

bool AIChats::prepareChat()
{
    if (!canPrepare()) {
        return false;
    }
    ...
    if (_activation_phase == ActivationPhase::CheckRequired ||
            _activation_phase == ActivationPhase::WaitingForBinding) {
        bool was_waiting =
            _activation_phase == ActivationPhase::WaitingForBinding;
        XiaozhiActivationClient::Info activation = {};
        esp_err_t ret = _activation_client->fetchInfo(activation);
        if (!canPrepare()) {
            return false;
        }
        if (ret != ESP_OK) {
            if (was_waiting) {
                ...
                setError(ret, "Get Xiaozhi activation information");
            }
            schedulePrepare(RETRY_INTERVAL_MS);
            return false;
        }
        ...
        if (activation.state ==
            ...
            copyActivation(
                activation.code.c_str(), activation.message.c_str()
            );
            _activation_phase = ActivationPhase::WaitingForBinding;
            setState(State::ActivationRequired);
            announceActivation(false);

            uint32_t delay_ms = activationRetryDelay(activation.timeout_ms);
            if (activation.challenge.empty()) {
            ...
            _activation_phase =
                was_waiting ? ActivationPhase::BoundPendingSuccess :
                ActivationPhase::Complete;
        }
    }

    if (_activation_phase == ActivationPhase::BoundPendingSuccess) {
        stopActivationPrompt();
        if (!canPrepare()) {
            return false;
        }
    ...
    setState(State::Preparing);
    XiaozhiActivationClient::Info service = {};
    esp_err_t ret = _activation_client->fetchInfo(service);
    if (!canPrepare()) {
        return false;
    }
    if (ret != ESP_OK) {
    ...
    if (service.state ==
            XiaozhiActivationClient::BindingState::ActivationRequired) {
        copyActivation(service.code.c_str(), service.message.c_str());
        _activation_phase = ActivationPhase::WaitingForBinding;
        setState(State::ActivationRequired);
        announceActivation(false);
        schedulePrepare(activationRetryDelay(service.timeout_ms));
        return false;
    }
    if (!service.has_websocket_config) {
    ...
    auto post_simple = [this](ControlEventType type) {
        ControlEvent event = {};
        event.type = type;
        event.chat_generation = _chat_generation.load();
        event.turn_generation = _turn_generation.load();
    ...
    XiaozhiProtocol::Callbacks callbacks = {};
    callbacks.onDisconnected = [this, post_simple]() {
        if (_chat_started.load() &&
                _channel_state.load() != ChannelState::Closing) {
            post_simple(ControlEventType::TransportDisconnected);
        }
    };
    callbacks.onServerGoodbye = [post_simple]() {
        post_simple(ControlEventType::ServerGoodbye);
    };
    callbacks.onText = [this](
        XiaozhiProtocol::TextRole role,
    ...
    callbacks.onEmotion = [this](const std::string &emotion) {
        ControlEvent event = {};
        event.type = ControlEventType::Emotion;
        event.chat_generation = _chat_generation.load();
        event.turn_generation = _turn_generation.load();
    ...
    callbacks.onTts = [this](
        XiaozhiProtocol::TtsState state,
        const std::string &text
    ) {
        (void)text;
        if (state == XiaozhiProtocol::TtsState::SentenceStart) {
            return;
        }
        ControlEvent event = {};
        event.type = state == XiaozhiProtocol::TtsState::Start ?
                     ControlEventType::TtsStart : ControlEventType::TtsStop;
        event.chat_generation = _chat_generation.load();
        event.turn_generation = _turn_generation.load();
        if (state == XiaozhiProtocol::TtsState::Start &&
                _channel_state.load() == ChannelState::Open &&
    ...
    callbacks.onAudio = [this](
        ...
        if (!data || len == 0 || len > sizeof(AudioPacket::data) ||
    ...
    XiaozhiProtocol::Config protocol_config(
        service.websocket_url,
        service.websocket_token,
        service.websocket_version,
        service.device_id,
        service.client_id,
        CHAT_SAMPLE_RATE,
        1,
        CHAT_FRAME_DURATION_MS,
        CONFIG_AICHATS_WEBSOCKET_HANDSHAKE_TIMEOUT_MS
    );
    auto *protocol = new (std::nothrow) XiaozhiProtocol(
...
void AIChats::destroyChat()
{
    ...
    XiaozhiProtocol *protocol = nullptr;
    if (_audio_tx_mutex &&
            xSemaphoreTake(_audio_tx_mutex, portMAX_DELAY) == pdTRUE) {
...
bool AIChats::ensureAudioReady()
{
    if (_audio_tasks_running.load() && _input_task_running.load() &&
            ...
            _opus_encoder && _opus_decoder && _input_resampler &&
    ...
    esp_err_t ret = bsp_extra_codec_set_voice_fs(
        DEVICE_AUDIO_SAMPLE_RATE, 16, 4, BIT(0)
    );
    if (ret != ESP_OK) {
    ...
    auto input_resampler_ret = esp_ae_rate_cvt_open(
        &input_resampler_config, &_input_resampler
    );
    if (input_resampler_ret != ESP_AE_ERR_OK || !_input_resampler ||
            esp_ae_rate_cvt_get_max_out_sample_num(
                _input_resampler,
                INPUT_READ_FRAMES,
                &_input_resampler_max_samples
            ) != ESP_AE_ERR_OK ||
            _input_resampler_max_samples == 0) {
        ESP_UTILS_LOGE(
...
bool AIChats::setAudioModes(bool wake_word, bool voice)
{
...
bool AIChats::enterReadyAudioMode()
{
...
void AIChats::finishPreroll(esp_err_t result)
{
...
esp_err_t AIChats::sendWakeWordPcm()
{
    if (!_audio_processor || !_audio_processor->isReady() ||
...
esp_err_t AIChats::openAudioChannel()
{
    XiaozhiProtocol *protocol = _protocol.load();
    if (!protocol) {
        return ESP_ERR_INVALID_STATE;
...
esp_err_t AIChats::startAutoStopTurn(const char *wake_word)
{
    ...
    if (!protocol || _channel_state.load() != ChannelState::Open) {
        ...
        if (ret == ESP_OK) {
            ret = protocol->sendStartListening(
                XiaozhiProtocol::ListeningMode::AutoStop
            );
        }
    }
    xSemaphoreGive(_audio_tx_mutex);
...
bool AIChats::startListening(ListenMode mode, const char *wake_word)
{
    ...
    if (auto_stop_mode) {
        ...
        if (ret == ESP_OK && _audio_tx_mutex &&
                xSemaphoreTake(_audio_tx_mutex, portMAX_DELAY) == pdTRUE) {
            ret = protocol->sendStartListening(
                XiaozhiProtocol::ListeningMode::ManualStop
            );
            xSemaphoreGive(_audio_tx_mutex);
        } else if (ret == ESP_OK) {
            ret = ESP_ERR_INVALID_STATE;
...
esp_err_t AIChats::stopUplink(bool notify_server)
{
    setAudioModes(false, false);
    if (!_audio_tx_mutex ||
            xSemaphoreTake(_audio_tx_mutex, portMAX_DELAY) != pdTRUE) {
...
void AIChats::cancelManualConversation()
{
    closeChannel(true);
    clearConversationUi();
    setState(State::Ready);
...
void AIChats::closeChannel(bool abort_speaking)
{
    setAudioModes(false, false);
    _accept_playback.store(false);
    _turn_generation.fetch_add(1);
        ...
        if (_audio_tx_mutex &&
            ...
            if (abort_speaking && (_tts_active ||
                ...
                (void)protocol->sendAbortSpeaking(
                    XiaozhiProtocol::AbortReason::None
                );
            }
...
void AIChats::resetChannelState()
{
    _uplink_ready.store(false);
...
void AIChats::handlePlaybackDrained()
{
    _accept_playback.store(false);
    bool ready = false;
    if (_playback_mutex &&
...
void AIChats::checkPlaybackDrained()
{
...
bool AIChats::playbackDrainReady() const
{
...
bool AIChats::resetPlaybackBuffer(bool reset_decoder)
{
    if (!_playback_mutex ||
            xSemaphoreTake(
...
bool AIChats::waitForPlaybackIdle(uint32_t timeout_ms) const
{
    TickType_t started = xTaskGetTickCount();
...
void AIChats::inputAudio()
{
    const size_t input_bytes = INPUT_READ_FRAMES * sizeof(int16_t);
    auto *device_mono = static_cast<int16_t *>(malloc(input_bytes));
    auto *chat_mono = static_cast<int16_t *>(
        malloc(_input_resampler_max_samples * sizeof(int16_t))
    );
    uint32_t read_failures = 0;
    uint32_t resample_failures = 0;
    if (!device_mono || !chat_mono || !_input_resampler) {
        free(device_mono);
        free(chat_mono);
        ControlEvent event = {};
    ...
    while (_audio_tasks_running.load()) {
        if (_input_resampler_reset_requested.exchange(false)) {
            (void)esp_ae_rate_cvt_reset(_input_resampler);
        }
        ...
        esp_err_t ret = bsp_extra_i2s_read(
            device_mono, input_bytes, &bytes_read, 100
        );
        if (ret == ESP_OK && bytes_read > 0 && _audio_processor) {
            read_failures = 0;
            const uint32_t input_samples =
                static_cast<uint32_t>(bytes_read / sizeof(int16_t));
            uint32_t peak = 0;
            uint32_t absolute_sum = 0;
            for (uint32_t i = 0; i < input_samples; ++i) {
                int32_t value = device_mono[i];
                uint32_t magnitude = static_cast<uint32_t>(
                    value < 0 ? -value : value
                );
                peak = magnitude > peak ? magnitude : peak;
            ...
            _input_pcm_mean_abs.store(
                input_samples ? absolute_sum / input_samples : 0
            );

            uint32_t output_samples = _input_resampler_max_samples;
            auto resample_ret = esp_ae_rate_cvt_process(
                _input_resampler,
                reinterpret_cast<esp_ae_sample_t>(device_mono),
                input_samples,
                reinterpret_cast<esp_ae_sample_t>(chat_mono),
                &output_samples
            );
            if (resample_ret == ESP_AE_ERR_OK && output_samples > 0 &&
                    output_samples <= _input_resampler_max_samples) {
                resample_failures = 0;
                _audio_processor->feed(chat_mono, output_samples);
            } else {
                ++resample_failures;
                if (resample_failures == 1 ||
                    ...
                    ESP_UTILS_LOGW(
                        ...
                        static_cast<unsigned long>(input_samples),
...
const char *AIChats::stateText(State state)
{
    switch (state) {
...
bool AIChats::tickReached(TickType_t now, TickType_t target)
{
    return static_cast<int32_t>(now - target) >= 0;
}

void AIChats::controlTaskEntry(void *arg)
{
    static_cast<AIChats *>(arg)->controlLoop();
}

void AIChats::inputTaskEntry(void *arg)
{
    static_cast<AIChats *>(arg)->inputAudio();
}

void AIChats::encoderTaskEntry(void *arg)
{
    static_cast<AIChats *>(arg)->encodeAudio();
}

void AIChats::playbackTaskEntry(void *arg)
{
    static_cast<AIChats *>(arg)->playAudio();
}

void AIChats::ipEventHandler(
    void *arg,
...
)
{
    ...
    (void)event_id;
    (void)event_data;
    auto *instance = static_cast<AIChats *>(arg);
    if (!instance) {
        return;
    }
...
void AIChats::uiTimerCallback(lv_timer_t *timer)
{
    auto *instance = static_cast<AIChats *>(lv_timer_get_user_data(timer));
    if (instance) {
        instance->refreshUi();
    }
}

void AIChats::uiActionCallback(void *context)
{
    auto *instance = static_cast<AIChats *>(context);
    if (!instance) {
        return;
    }
    ControlEvent event = {};
