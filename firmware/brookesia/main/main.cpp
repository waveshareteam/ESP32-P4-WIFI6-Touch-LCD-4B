...
extern "C" void app_main(void)
{
...

    {
        ...
        const auto &custom_stylesheet = ESP_BROOKESIA_PHONE_CUSTOM_STYLESHEET();
        ESP_UTILS_CHECK_FALSE_EXIT(phone->addStylesheet(custom_stylesheet), "Add custom stylesheet failed");
        ESP_UTILS_CHECK_FALSE_EXIT(phone->activateStylesheet(custom_stylesheet), "Activate custom stylesheet failed");
    ...
    refresh_boot_loading(boot_loading, "Loading hardware services...", 90);
    {
        auto ethernet_app = esp_brookesia::apps::EthernetInfo::requestInstance();
        ESP_UTILS_CHECK_FALSE_EXIT(phone->installApp(ethernet_app), "Start Ethernet failed");
        auto relay_app = esp_brookesia::apps::RelayControl::requestInstance();
        ESP_UTILS_CHECK_FALSE_EXIT(phone->installApp(relay_app), "Start Relay Control failed");
        auto rs485_app = esp_brookesia::apps::RS485Terminal::requestInstance();
        ESP_UTILS_CHECK_FALSE_EXIT(phone->installApp(rs485_app), "Start RS485 failed");
    }
